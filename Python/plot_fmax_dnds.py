...
for treatment in treatments:
    ...
    for taxon in taxa:
        ...
        max_fmax_all.append(min([max(synonymous_fmax_all), max(nonsynonymous_fmax_all)]))
        min_fmax_all.append(max([min(synonymous_fmax_all), min(nonsynonymous_fmax_all)]))


        fmax_dict[treatment][taxon] = {}
        fmax_dict[treatment][taxon]['synonymous_fmax'] = np.asarray(synonymous_fmax_all)
        fmax_dict[treatment][taxon]['nonsynonymous_fmax'] = np.asarray(nonsynonymous_fmax_all)
...
for treatment_idx, treatment in enumerate(treatments):
    ...
    ax = fig.add_subplot(gs[0, treatment_idx])
    ax.set_title(pt.treatment_label_dict[treatment], fontsize=12, fontweight='bold' )
    ...
    fmax_dict_treatment = fmax_dict[treatment]

    fmax_cutoff_dict[treatment] = {}

    for fmax_dict_treatment_key in fmax_dict_treatment.keys():

        fmax_dict_treatment_taxon_dict = fmax_dict_treatment[fmax_dict_treatment_key]

        nonsynonymous_fmax = fmax_dict_treatment_taxon_dict['nonsynonymous_fmax']
        synonymous_fmax = fmax_dict_treatment_taxon_dict['synonymous_fmax']
        ...
        for fmax_max in fmax_max_range:
            ...
            pNpS = (n_nonsynonymous_fmax_max/n_synonymous_fmax_max) * ((Lsyn+1)/ (Lnon+1))

            fmax_max_passed.append(fmax_max)
            pNpS_fmax.append(pNpS)

        pNpS_fmax = np.asarray(pNpS_fmax)
        fmax_cutoff_dict[treatment][fmax_dict_treatment_key] = pNpS_fmax

        ax.plot(fmax_max_passed, pNpS_fmax, c =pt.get_colors(treatment), linewidth=1.5, ls = pt.linestyle_dict[fmax_dict_treatment_key], zorder=2)
