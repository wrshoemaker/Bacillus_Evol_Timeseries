...
for taxon in ['B', 'S']:
    ...
    for treatment in treatments:
        ...
        G_subsample_list = np.asarray(G_subsample_list)
        ...
        print(taxon, treatment, G, pvalue)
...
for treatment in treatments:
    ...
    B_values = G_subsample_dict[treatment+'B']
    S_values = G_subsample_dict[treatment+'S']

    #merge = np.vstack((B_values,S_values))
    #merge = merge.T

    merge = np.concatenate((B_values, S_values))

    t = np.mean( np.absolute(merge[:len(B_values)] - merge[len(B_values):] ) )

    t_k_list = []
    for k in range(10000):

        np.random.shuffle(merge)
        t_k = np.mean( np.absolute(merge[:len(B_values)] - merge[len(B_values):])  )
        t_k_list.append(t_k)

    t_k_list = np.asarray(t_k_list)

    P = (sum(t_k_list>t) + 1) / (len(t_k_list)+1)



    print(str(10**int(treatment)) + '-day: t = ' + str(round(t, 3)) + ', P = ' + str(round(P, 5)))


    #merge = merge[np.random.permutation(merge.shape[0]),:]
    #merge = merge[:,np.random.permutation(merge.shape[1])]
