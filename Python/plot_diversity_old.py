...
for treatment_idx, treatment in enumerate(treatments):
    ...
    for taxon_i, taxon in enumerate(taxa):
        ...
        #print(treatment, taxon, b0, 'K',  K, '+/-', ses_K)
        #print(treatment, taxon, b0, 'v_0',  v_0, '+/-', ses_v_0)
...
for taxon in taxa:
    ...
    for treatment in treatments:
        ...
        x_ = param_dict[taxon][treatment]['V_max']
        y_ = param_dict[taxon][treatment]['K_gen']

        ax.scatter(x_, y_, s = 180, \
            linewidth=3, facecolors=pt.get_scatter_facecolor(taxon, treatment), \
            edgecolors=pt.get_colors(treatment), marker=pt.plot_species_marker(taxon), \
            alpha=0.6, zorder=2)
...
fig_name = pt.get_path() + '/figs/parameters.jpg'
fig.savefig(fig_name, format='jpg',  bbox_inches = "tight", pad_inches = 0.4, dpi = 600)
plt.close()
